...
module pod_demand
	...
	//[do_not] true -> (pod'=current_pod);

	[] pod>0 & pod<=50 ->0.185:(demand'=demand)&(l'=lat_a)&(ud'=util_a)// &(pod'=pod)  
				+ 0.630:(demand'=demand_b)&(l'=lat_b)&(ud'=util_b)// &(pod'=pod_a) 
		 	 	+ 0.185:(demand'=demand_c)&(l'=lat_c)&(ud'=util_c);// &(pod'=pod_b); 

	[] pod>50 & pod<=limitPod ->0.185:(demand'=demand_d)&(l'=lat_d)&(ud'=util_d)// &(pod'=pod_c)
				+ 0.630:(demand'=demand)&(l'=lat_e)&(ud'=util_e)// &(pod'=pod_d)
		 	 	+ 0.185:(demand'=demand_e)&(l'=lat_f)&(ud'=util_f); // &(pod'=desired_replica); 
endmodule
...
module autoscaler
	...
	[scale_out] (60>=cpu & cpu<=100) & (current_pod<desired_replica) & (t+TAU<maxTime) ->  1/2:(act'=0)&(current_pod'=desired_replica<maxPod?current_pod+current_pod:maxPod)
	&(t'=t+TAU)&(util'=cpu>util? u_util:cpu)+ 1/2:(act'=0)&(current_pod'=desired_replica<40?current_pod+4:current_pod+current_pod)&(t'=t+TAU)&(util'=cpu>util?u_util:cpu);

	[scale_in] (0>=cpu & cpu<60)& (current_pod>maxPod) & (t+TAU<maxTime) -> (act'=1)&(current_pod'=desired_replica<minPod?minPod:minPod)&(t'=t+TAU)&(util'=cpu>util? u_util:cpu);

	//no scaling, update current number of pods, receive new demand & utilization
	[do_not] (40>=cpu & cpu<60) | (t=maxTime) | (current_pod=maxPod) | (current_pod=desired_replica)-> (act'=2)&(current_pod'=current_pod)&(util'=util);
