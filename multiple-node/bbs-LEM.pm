...
const init_pod; const init_demand; const init_pow; const init_rt; const TAU=2;	
												//initial number of pods, utilization, response time, period duration
...
formula desired_replica = ceil(pod*(u/target_util)); 	
...
module autoscaler
	...
	[scale_out] (60>=u & u<=100)  & (pod<desired_replica) & (t+TAU<maxTime) -> 1/2:(current_pod'=desired_replica<maxPod?pod+pod:maxPod)
							&(t'=t+TAU)&(util'=u>util? u_util:util) + 1/2:(current_pod'=desired_replica<40?pod+4:pod+pod)
	...
	(current_pod'=desired_replica<minPod?minPod:minPod)&(lat'=-1)&(t'=t+TAU)&(util'=u>util? u_util:util);
	
	[scale_out] (lat=-1) & l>0 & (rt>maxRt) & (t+TAU<maxTime)-> 1/2:(current_pod'=desired_replica<maxPod?pod+pod:maxPod)&(lat'=l>0?l-1:lat)&(rt'=up_rt)&(util'=u>util? u_util:util)&(t'=t+TAU)
		+1/2:(current_pod'=desired_replica<40?pod+4:pod+pod)&(lat'=l>0?l-1:lat)&(rt'=up_rt)&(util'=u>util? u_util:util)&(t'=t+TAU);
	

	//energy
	[scale_in] (pow>maxPower) & (pod>maxPod) & (t+TAU<maxTime) -> (current_pod'=desired_replica<minPod?minPod:minPod)&(t'=t+TAU)&(util'=u>util? u_util:util);
	[scale_in] (power>300)  & (current_pod>maxPod) & (t+TAU<maxTime) -> (current_pod'=desired_replica<minPod?minPod:minPod)&(t'=t+TAU)&(util'=u>util? u_util:util);
