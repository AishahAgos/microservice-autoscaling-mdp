...
module pod_demand
	...
	[] pod>0 & pod<=50 ->0.185:(demand'=demand)&(l'=lat_a)&(ud'=util_a)
				+ 0.630:(demand'=demand)&(l'=lat_b)&(ud'=util_b)
		 	 	+ 0.185:(demand'=demand)&(l'=lat_c)&(ud'=util_c);

	[] pod>50 & pod<=limitPod ->0.185:(demand'=demand)&(l'=lat_d)&(ud'=util_d)
				+ 0.630:(demand'=demand)&(l'=lat_e)&(ud'=util_e)
		 	 	+ 0.185:(demand'=demand)&(l'=lat_f)&(ud'=util_f); 
endmodule
...
module autoscaler
	...
	//energy
	[scale_in] (pow>maxPower) & (current_pod>maxPod) & (t+TAU<maxTime) -> (act'=1)&(current_pod'=desired_replica<minPod?minPod:minPod)&(t'=t+TAU)&(util'=cpu>util? u_util:cpu);
	[scale_in] (power>300)  & (current_pod>maxPod) & (t+TAU<maxTime) -> (act'=1)&(current_pod'=desired_replica<minPod?minPod:minPod)&(t'=t+TAU)&(util'=cpu>util? u_util:cpu);
